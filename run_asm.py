...
from preprocess import extract_asm, args_parser
from entities import convert_examples_to_features, read_examples_asm
import argparse
...
def main():
    ...
    if args.do_train:
        ...
        train_examples = read_examples_asm(train_dataset_dir)
        ...
        for step in bar:
            ...
            if args.do_eval and ((global_step + 1) %args.eval_steps == 0) and eval_flag:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples = read_examples_asm(dev_dataset_dir)
    ...
    if args.do_test:
        sample_dict = extract_asm(test_dataset_dir)
        for idx in tqdm(sample_dict, total=len(sample_dict)):
            logger.info("Test function: {}".format(idx))
            eval_examples = read_examples_asm(sample_dict, idx=idx)
            eval_features = convert_examples_to_features(eval_examples, tokenizer, args,stage='test')
            all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
            all_source_mask = torch.tensor([f.source_mask for f in eval_features], dtype=torch.long)    
            eval_data = TensorDataset(all_source_ids,all_source_mask)   
